...
plot_max:
	mov	bl, 0x0
	mov	byte [es:di], bl	; If i = max_i, plot black pixel

continue:

	push	di		; Temporarily move the indices, row, and ax to the stack
	push	si
	push	ax
	push 	dx

	mov	ax, dx		; Get the row value
	mov	dx, 320
	mul	dx
	mov	di, 0xFA00
	sub	di, ax
	...
	mov	byte [es:di], bl	; Fill pixel reflected about the x axis

	pop	dx		; Return values from the stack
	pop	ax
	pop	si
	pop	di
	...
	inc	di
	inc	cx
	cmp	di, 0x7E40	; Escape loop if end of screen is reached
	je	exit
